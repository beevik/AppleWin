...
Update_t _CmdSymbolsListTables(int nArgs, int bSymbolTables) {
    ...
    */
    ...
    //      bDisplaySymbolTables = (1 << iWhichTable);
            ...
            if (nAddress) {   // Have address, do symbol lookup first
                ...
                if (!_CmdSymbolList_Symbol2Address(pSymbol, bSymbolTables)) {   // nope, ok, try as address
...
//===========================================================================
Update_t _CmdSymbolsCommon(int nArgs, SymbolTable_Index_e bSymbolTables) {
    if (!nArgs) {
...
Update_t CmdSymbolsCommand(int nArgs) {
    ...
    SymbolTable_Index_e index = (SymbolTable_Index_e)(SYMBOL_TABLE_MAIN << GetSymbolTableFromCommand());
    return _CmdSymbolsCommon(nArgs, index);
}
