...
PageSound * PageSound::ms_this = 0;    // reinit'd in ctor

const TCHAR PageSound::m_soundchoices[] = TEXT("Disabled\0")
TEXT("Sound Card\0");


BOOL CALLBACK PageSound::DlgProc(HWND hWnd, UINT message, WPARAM wparam, LPARAM lparam) {
    // Switch from static func to our instance
    return PageSound::ms_this->DlgProcInternal(hWnd, message, wparam, lparam);
}

BOOL PageSound::DlgProcInternal(HWND hWnd, UINT message, WPARAM wparam, LPARAM lparam) {
    switch (message) {
    case WM_NOTIFY:
    {
...
void PageSound::DlgOK(HWND hWnd) {
...
void PageSound::InitOptions(HWND hWnd) {
    // CheckRadioButton
    ...
    // Mockingboard button
    {
        const BOOL bEnable = (bIsSlot4Empty || Slot4 == CT_Phasor || Slot4 == CT_MockingboardC) &&
            (bIsSlot5Empty || Slot5 == CT_MockingboardC);
...
bool PageSound::NewSoundcardConfigured(HWND hWnd, WPARAM wparam, SS_CARDTYPE NewCardType) {
